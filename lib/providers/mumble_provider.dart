...
import 'package:push_to_talk_app/providers/connection_options.dart';
import 'package:push_to_talk_app/providers/log/mumble_log.dart';
...
class MumbleProvider extends ChangeNotifier with NetworkLoggy {
  ...
  Future<void> connect({
    ...
    return MumbleClient.connect(
      options: await createConnectionsOptionsWithCertificate(connectionOptions),
  ...
  void startTransmit() async {
    ...
    if (await Permission.microphone.request().isGranted && client != null) {
      ...
      AudioFrameSink audioOutput = client!.audio.sendAudio(
        codec: AudioCodec.opus,
      );

      var recordingDataController = StreamController<Food>();
      Stream<Uint8List>? audioStream = recordingDataController.stream.map(
        (event) => (event as FoodData).data!,
      );
  ...
  void onUserRemoved(User user, User? actor, String? reason, bool? ban) {
    ...
    if (actor == null || user == actor) {
      ...
      mumbleLog.user(
        '${user.name} was banned by ${actor.name}, reason $reason.',
      );
    } else {
      // The user was kicked from the server
      mumbleLog.user(
        '${user.name} was kicked by ${actor.name}, reason $reason.',
      );
    }
  }
