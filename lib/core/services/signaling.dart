...
typedef StreamStateCallback =
    void Function(String peerUuid, String displayName, MediaStream? stream);
...
class Signaling {
  ...
  bool isMicEnabled() {
    if (_localStream != null) {
      return _localStream!.getAudioTracks().any((c) => c.enabled);
    }
  ...
  bool isCameraEnabled() {
    if (_localStream != null) {
      return _localStream!.getVideoTracks().any((c) => c.enabled);
    }
  ...
  Future<RTCPeerConnection> _createPeerConnection(
    ...
    pc.onIceCandidate = (event) =>
        _gotIceCandidate(event, fromPeerId, displayName);
    pc.onTrack = (event) => _gotRemoteStream(event, fromPeerId, displayName);
    pc.onIceConnectionState = (event) =>
        _checkConnectionState(event, fromPeerId, displayName);
  ...
  Future<void> _clearAllFirebaseData() async {
    ...
    await FirebaseFirestore.instance
        ...
        .doc(_appointmentId)
        .collection(tablePeers)
        .where('uuid', isEqualTo: _localUuid)
        .get()
        .then((snapshot) async {
